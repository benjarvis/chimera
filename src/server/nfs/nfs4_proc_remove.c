    ...
    void                           *private_data)
{
    ...
    /* Only pay for the extra LOOKUP when pNFS is in play; otherwise remove
     * straight away (have_layout stays 0). */
    if (chimera_vfs_pnfs_enabled(req->thread->shared->vfs)) {
        chimera_vfs_lookup_at(req->thread->vfs_thread, &req->cred,
                              parent_handle,
                              args->target.data, args->target.len,
                              CHIMERA_VFS_ATTR_PNFS_LAYOUT | CHIMERA_VFS_ATTR_NLINK |
