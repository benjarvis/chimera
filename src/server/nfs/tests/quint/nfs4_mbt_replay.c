    ...
    struct mism   *m)
{
    ...
    /* RDMA chunk eligibility for this compound, mirroring what chimera's own
     * v4 client asks for (vfs/nfs/nfs4_write.c, nfs4_read.c): a WRITE puts its
     * payload in a Read chunk for the server to pull (ddp), and a READ
     * advertises a Write chunk the server can place the reply data into.
     * Without these an RDMA run gets RPC-over-RDMA framing but never a
     * one-sided transfer, and the server's chunk handling goes untested. */
    int cd_ddp = 0, cd_wchunk = 0;
