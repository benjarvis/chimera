    ...
    if (request->ioctl.input_count > 0) {
        ...
        switch (request->ioctl.ctl_code) {
            ...
            case SMB2_FSCTL_SRV_COPYCHUNK_WRITE: {
                 ...
                 * Dropping it here (return -1) would leave the client hanging,
                 * and parsing cc entries would overrun cc_chunks[], so skip the
                 * chunk array entirely and let the handler reject by count. */
