    ...
    int                               received_encrypted)
{
 ...
 next_compound_request:
        ...
        if (more_requests) {
            /* NextCommand was validated above (aligned, forward, in-bounds) and
             * the parser cannot have read past it, so this skip is non-negative
             * and within the received data; use the checked variant anyway so a
             * future regression rejects the chain instead of aborting. */
