...
/* Forward declaration for memfs_inode_free */
static void memfs_xattr_release(struct rb_node *node, void *private_data);
    ...
    struct memfs_inode  *inode)
{
    ...
    rb_tree_destroy(&inode->xattrs, memfs_xattr_release, thread);
...
{
    if (xattr->key_len + xattr->value_len <= 512) {
        LL_PREPEND(thread->free_xattr, xattr);
    } else {
        free(xattr);
    }
} /* memfs_xattr_free */
...
{
    struct memfs_thread *thread = private_data;
    struct memfs_xattr  *xattr  = container_of(node, struct memfs_xattr, node);

    if (thread) {
        memfs_xattr_free(thread, xattr);
    } else {
        free(xattr);
    }
} /* memfs_xattr_release */
...
{
    struct memfs_xattr *xattr;
    size_t              size = sizeof(struct memfs_xattr) + key_len + value_len;

    xattr = thread->free_xattr;

    if (xattr && size <= sizeof(struct memfs_xattr) + 512) {
        LL_DELETE(thread->free_xattr, xattr);
    } else {
        xattr = (struct memfs_xattr *) malloc(size);
    }
    ...
    for (i = 0; i < shared->num_inode_list; i++) {
        for (j = 0; j < shared->inode_list[i].num_blocks; j++) {
            for (k = 0; k < CHIMERA_MEMFS_INODE_BLOCK; k++) {
                ...
                rb_tree_destroy(&inode->xattrs, memfs_xattr_release, NULL);
            }
...
memfs_thread_destroy(void *private_data)
{
    ...
    while (thread->free_xattr) {
        struct memfs_xattr *xattr = thread->free_xattr;
        LL_DELETE(thread->free_xattr, xattr);
        free(xattr);
    }
    ...
    void                       *private_data)
{
    ...
    inode = (struct memfs_inode *) request->getxattr.handle->vfs_private;
    ...
    memfs_map_attrs(&request->getxattr.r_attr, inode);
...
{
    struct memfs_inode      *inode;
    struct memfs_xattr      *xattr, *old_xattr;
    struct evpl_iovec_cursor cursor;
    const char              *key;
    char                    *value_buf;
    ...
    inode = (struct memfs_inode *) request->setxattr.handle->vfs_private;
    ...
    memfs_map_attrs(&request->setxattr.r_pre_attr, inode);
    ...
    if (old_xattr && old_xattr->key_len == key_len &&
        ...
        memfs_xattr_free(thread, old_xattr);
    } else {
    ...
    /* Create new xattr */
    xattr = memfs_xattr_alloc(thread, key, key_len, value_buf, value_len, hash);
    rb_tree_insert(&inode->xattrs, hash, xattr);

    memfs_map_attrs(&request->setxattr.r_post_attr, inode);
    ...
    void                       *private_data)
{
    ...
    inode = (struct memfs_inode *) request->removexattr.handle->vfs_private;
    ...
    memfs_map_attrs(&request->removexattr.r_pre_attr, inode);
    ...
    memfs_map_attrs(&request->removexattr.r_post_attr, inode);
    ...
    void                       *private_data)
{
    ...
    inode = (struct memfs_inode *) request->listxattr.handle->vfs_private;
    ...
    memfs_map_attrs(&request->listxattr.r_attr, inode);
    ...
    void                       *private_data)
{
    ...
    switch (request->opcode) {
        ...
        case CHIMERA_VFS_OP_GETXATTR:
            memfs_getxattr(thread, shared, request, private_data);
            break;
        case CHIMERA_VFS_OP_SETXATTR:
            memfs_setxattr(thread, shared, request, private_data);
            break;
        case CHIMERA_VFS_OP_REMOVEXATTR:
            memfs_removexattr(thread, shared, request, private_data);
            break;
        case CHIMERA_VFS_OP_LISTXATTR:
            memfs_listxattr(thread, shared, request, private_data);
            break;
        default:
