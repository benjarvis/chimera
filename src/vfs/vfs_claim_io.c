 ...
 * ticket stays queued across pump/resume (the wait flag below marks
 * "resume posted") and is only unlinked when its request finally proceeds
 * or fails, so io_try can refuse to let later same-file I/O leapfrog it.
 * Protocol streams depend on same-thread same-file completions arriving in
 * submission order (S3 GET appends read chunks to the HTTP response in
 * completion order); without the barrier, the backend-cover gate's
 * first-I/O park lets chunks submitted after the cover lands complete
 * ahead of the parked ones, scrambling the stream (rocky boto3
 * multipart/streaming content mismatches).
 *
 * A request re-parking after a resume attempt therefore KEEPS ITS
 * POSITION; only the wait flag is cleared so a future pump re-posts it. */
static void
...
/* Unlink `request`'s ticket from the wait queue if present.  Caller holds
 * file->lock.  Returns true when it was queued: the caller must then pump
 * (outside the lock) so the next-in-line waiter blocked on the departing
 * ticket's barrier position is re-posted. */
static bool
    ...
    struct chimera_vfs_request    *request)
{
    ...
    if (ticket->prev) {
        ...
        file->io_wait_head = ticket->next;
    }
    if (ticket->next) {
        ticket->next->prev = ticket->prev;
    } else {
        file->io_wait_tail = ticket->prev;
    }
    ticket->prev   = NULL;
    ticket->next   = NULL;
    ticket->queued = false;
 ...
 * a posted resume so a ticket is never double-posted onto its thread's
 * pending_io_resume list; the resumed request either proceeds (unparking
 * itself and pumping the next in line) or re-parks in place with wait
 * cleared, to be re-posted by the next pump. */
void
chimera_vfs_claim_pump_io(
    struct chimera_vfs_state      *state,
    struct chimera_vfs_file_state *file)
{
#define CHIMERA_VFS_IO_PUMP_BATCH 64
    struct chimera_vfs_request         *reqs[CHIMERA_VFS_IO_PUMP_BATCH];
    ...
    } while (n == CHIMERA_VFS_IO_PUMP_BATCH);
#undef CHIMERA_VFS_IO_PUMP_BATCH
} /* chimera_vfs_claim_pump_io */
    ...
    struct chimera_vfs_request    *request)
{
     ...
     * queue HEAD is exempt so the front of the line can always make
     * progress and drive the barrier forward). */
